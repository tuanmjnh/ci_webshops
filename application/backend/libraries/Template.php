...
class Template
{
	...
	public function set_metadata($name, $content, $type = 'meta')
	{
        ...
        public function prepend_asset($line)
	{
		array_unshift($this->_asset, $line);
		return $this;
	}
        public function append_asset($line)
	{
		$this->_asset[] = $line;
		return $this;
	}
/**
	 * Set asset for output
	 *
	 */
	public function set_asset($data, $url = 'assets/') {

        $url = TMUrl::TM_BASE_URL() . $url;
        if (preg_match('/css$/i', $data)) 
            $this->_asset[$data] = '<link href="' . $url . $data . '" rel="stylesheet">';
         elseif (preg_match('/js$/i', $data)) 
            $this->_asset[$data] = '<script type="text/javascript" src="' . $url . $data . '"></script>';
         else 
            $this->_asset[$data] = '<link rel="icon" href="' . $url . $data . '" type="image/x-icon"/>';
